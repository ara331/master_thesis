...
void action_evaluation(proj C, const uint8_t key[], const proj A)
{
		...
		for(i = 0; i < size_of_each_batch[m]; i++)
		{
			...
			else
			{
				...
				if ((isinfinity(G[0]) != 1))	// Depending on randomness
				{
					...
					if (isog_counter==MID_NUMBER_OF_ISOGENIES){
	...
	// //counterも変更
	memcpy(counter, B1, sizeof(int8_t) * N);
	...
	while (isog_counter < number_of_isogenies_1+number_of_isogenies_2)
	{
		...
		for(i = 0; i < size_of_each_batch[m]; i++)
		{
			...
			else
			{
				...
				ec2 = lookup(batches[m][i], mid_r);

				bc1 = isequal(ec1 >> 1, 0) & 1;		// Bit that determines if a dummy operation will be perfomed
				bc2 = isequal(ec2 >> 1, 0) & 1;		// Bit that determines if a dummy operation will be perfomed

				fp_cswap(current_T[0][0], current_T[1][0], (ec1 & 1));		// constant-time swap: T_{+} or T_{-}, that is the question.
				fp_cswap(current_T[0][1], current_T[1][1], (ec1 & 1));
				fp_cswap(current_T[2][0], current_T[3][0], (ec2 & 1));		// constant-time swap: T_{+} or T_{-}, that is the question.
				fp_cswap(current_T[2][1], current_T[3][1], (ec2 & 1));		// constant-time swap: T_{+} or T_{-}, that is the question.
				
				//追加　Gを入れ替えて今から計算しないといけないので，Aも入れ替えないといけない．
				fp_cswap(current_A[0][0], current_A[1][0], bc1);	// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_A[0][1], current_A[1][1], bc1);	// constant-time swap: dummy or not dummy, that is the question.
				
				//追加 bcによって曲線を入れ替えてしまったので，これが必要か．　
				fp_cswap(current_T[0][0], current_T[2][0], bc1);		// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_T[0][1], current_T[2][1], bc1);		// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_T[1][0], current_T[3][0], bc1);		// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_T[1][1], current_T[3][1], bc1);		// constant-time swap: dummy or not dummy, that is the question.
				...
				//使わない曲線上の点をl倍しなければ，次のループでうまく使えない
				...
				if (isinfinity(G[0]) != 1)	// Depending on randomness
				{
					...
					mid_r[batches[m][i]] = ((((ec2 >> 1) - (bc1 & (bc2 ^ 1))) ^ (bc1 & bc2)) << 1) ^ ((ec2 & 0x1) ^ (bc2 & bc1));
				...
				{
					//isogeny計算が行われなかったときも，反対の正負の点はl倍しておかないといけない．               
					yMUL(current_T[1], current_T[1], current_A[0], batches[m][i]);
				};
				...
				fp_cswap(current_T[0][0], current_T[1][0], (ec1 & 1));		// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_T[0][1], current_T[1][1], (ec1 & 1));		// constant-time swap: dummy or not dummy, that is the question.

				fp_cswap(current_T[2][0], current_T[3][0], (ec2 & 1));		// constant-time swap: dummy or not dummy, that is the question.
				fp_cswap(current_T[2][1], current_T[3][1], (ec2 & 1));		// constant-time swap: dummy or not dummy, that is the question.
	...
	fp_inv(C1[1]);
	fp_mul(C1[0], C1[0], C1[1]);
	fp_inv(C3[1]);
	fp_mul(C3[0], C3[0], C3[1]);
	//E0-E0
	fp_sub(C1[0],C1[0],C3[0]);

	//E+0
	fp_add(C[0],C2[0],C1[0]);
	fp_add(C[1],C2[1],C1[0]);
	...
	// printf("mid_rの残り=[");
    // for(int ii=0;ii<=73;ii++){
	// 	k = (int)( (2*(mid_r[ii] & 0x1) - 1) * (mid_r[ii] >> 1) );
    //     printf("%d,",k);
    // }
    // printf("]\n");
...
void action_evaluation(proj C, const uint8_t key[], const proj A)
{
	...
	for (int ii=0; ii<=73; ii++){
		...
		number_of_isogenies_1=number_of_isogenies_1+exp1;
		number_of_isogenies_2=number_of_isogenies_2+k2+(sgn1+1)*(k3 + (k3%2));
	...
	// printf("mid_rの残り=[");
    // for(int ii=0;ii<=73;ii++){
	// 	k1 = (int)( (2*(mid_r[ii] & 0x1) - 1) * (mid_r[ii] >> 1) );
    //     printf("%d,",k1);
    // }
    // printf("]\n");
